...
-- In a nutshell, for multiplayer maps, different gamemodes are different subworlds, each referenced in the main level partition (e.g. Levels/XP2_Palace/XP2_Palace).
-- This script clones the SubWorldReferenceObjectData from the XP2_Palace main level partition, and adds it to the singleplayer/co-op map being loaded.
...
Events:Subscribe('Partition:Loaded', function(partition)
    ...
    if string.find(levelName, 'XP2_Palace') == nil or gameModeName ~= 'AirSuperiority0' then
        return
    end
...
Events:Subscribe('Partition:Loaded', function(partition)
    ...
    -- Don't continue if the level is not any singleplayer or coop level in TDM CQ.
    -- Again, change this to have the exact same code as on line 45 of MpDataLoad.lua, so that this code only runs when we're loading the map and gamemodes we want.
    if string.find(levelName, 'XP2_Palace') == nil or gameModeName ~= 'SquadRush0' then
        return
    end
...
Events:Subscribe('Partition:Loaded', function(partition)
    ...
    -- Don't continue if the level is not any singleplayer or coop level in TDM CQ.
    -- Again, change this to have the exact same code as on line 45 of MpDataLoad.lua, so that this code only runs when we're loading the map and gamemodes we want.
    if string.find(levelName, 'XP2_Palace') == nil or gameModeName ~= 'CaptureTheFlag0' then
        return
    end
